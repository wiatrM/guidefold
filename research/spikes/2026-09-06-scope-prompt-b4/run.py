...
def main():
 ...
 manifest={'created_utc':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'skill_source_sha256':e.sha(sf),'skills':[s['id'] for s in chosen],'audit_skill_ids':audit,'n':32,'arms':['original_prompt','scoped_task_prompt'],'same_source_and_decoder':True,'baseline_prompt_sha256':e.h(e.SYSTEM),'scoped_prompt_sha256':e.h(e.SYSTEM+ADD),'parent_script_sha256':e.sha(BASE/'run.py'),'script_sha256':e.sha(Path(__file__)),'model':str(e.MODEL),'max_new_tokens':448,'batch_size':4,'seed':e.SEED,'no_real_queries_or_qrels_read':True,'retrieval_results_not_used':True}
