...
|---|---|---|
| Overview (`/home`, od 2026-09-12 domyślny cel po zalogowaniu i nieznanych adresów) | `usage`, `skills` (limit 100, maksimum kontraktu), `skills/facets` (scope), `map/layers`, `proposals`, `imports`, `installations`, `audit` | Osiem odczytów równolegle; każdy blok, którego odczyt się nie powiódł, mówi to sam, reszta strony jest kompletna. Domena `src/domain/overview.ts` (czyste agregacje, testy null/zero/partial; `delta`/`yourDecisions` od 1.3.0). `audit` czytany dla każdej roli — member dostaje od serwera tylko własne wiersze (§4.1), owner każdy; karta „Recent activity” ma odpowiedni eyebrow. Karty KPI (exposures, verified loads, helped share) noszą plakietkę trendu względem `usage.previous.totals`, kiedy API je zwróci (`+N%`/`-N%`/`0%`); bez `previous`, albo gdy jedno z dwóch okien liczy zero, plakietka mówi „No previous window”, nigdy fałszywy procent — kolejka („Needs review”) nie ma odpowiednika w poprzednim oknie i zawsze pokazuje ten sam napis. Karta „Your decisions” (liczba + do pięciu ostatnich, `decision.actor === me.user.id`, propozycje i pozycje kolejki razem, najnowsze pierwsze) pojawia się tylko, gdy ten licznik jest większy od zera. Od 2026-09-13 (ADR-0047, kontrakt 1.11.0 §4.10) odczyty idą do `{org_base}` gdy `?repo=` jest puste — cała organizacja w zasięgu czytającego — a do `{repo_base}` po wyborze repozytorium w railu; wiersze niosą `repo_id`. Bez repozytorium „Largest scopes” staje się „Largest repositories” (`skills/facets?field=repo`). Od 1.12.0 (§4.10 pkt 9) karta Library czyta `skills/duplicates` tylko w zakresie organizacji i dopisuje linię „N skill names appear in more than one repository” z linkiem do `library?duplicates=1`, gdy N > 0 (długość pierwszej strony, `+` gdy jest `next_cursor`). |
...
| Library | `skills`, `skills/facets`, `skills/facets/lookup`, `skills/duplicates` | Filtry `q, scope, owner, layer, status` i `cursor` w URL; bez pobierania body. Aktywna wartość ma własny lookup; `filters[*].available:false` renderuje jawny brak, nigdy cichego „All”. Od 2026-09-13 (ADR-0047, kontrakt 1.11.0 §4.10) odczyty idą do `{org_base}` gdy `?repo=` jest puste — cała organizacja w zasięgu czytającego — a do `{repo_base}` po wyborze repozytorium w railu; wiersze niosą `repo_id`. Od 1.12.0 panel „Duplicated across repositories” nad listą pokazuje do 5 nazw obecnych w więcej niż jednym repozytorium (repozytoria, `Identical`/`Differs`, linki do każdego członka z jego `repo`); „Show all N” ustawia `?duplicates=1`, które zastępuje listę skilli pełną, stronicowaną listą grup. |
| Map | `map/repository`, `map/scopes`, `map/layers`, `map/relations`, `modules/{scope}` | Leniwe rozwijanie gałęzi (≤100 obiektów na żądanie, twardy limit renderu 200), tekstowa lista relacji z etykietą typu, jawne `truncated`, nazwany unmapped scope. Od 2026-09-13 (ADR-0047, kontrakt 1.11.0 §4.10) odczyty idą do `{org_base}` gdy `?repo=` jest puste — cała organizacja w zasięgu czytającego — a do `{repo_base}` po wyborze repozytorium w railu; wiersze niosą `repo_id`. Korzeń drzewa bez repozytorium to lista repozytoriów (`kind: repository`); wybór scope'u dopisuje `repo` do adresu (identyfikator scope'u jest unikalny w repozytorium; `scope_ambiguous` przy bezpośrednim linku). |
| Skill | `skills/{id}`, `revisions/{rev}`, `revisions/{rev}/raw`, `revisions/{rev}/feedback` | Brak rewizji to `revision_not_found`, nigdy podmiana na nowszą. Link źródła buduje się z `source.url`; bez `git_host_url` pokazujemy ścieżkę i „Source host not configured”. Od 2026-09-13 (ADR-0047, kontrakt 1.11.0 §4.10) odczyty idą do `{org_base}` gdy `?repo=` jest puste — cała organizacja w zasięgu czytającego — a do `{repo_base}` po wyborze repozytorium w railu; wiersze niosą `repo_id`. Feedback idzie do `{repo_base}` z `repo_id` skilla. |
| Proposals | `proposals`, `proposals/{id}`, `decision`, `export`, `publication`, `snapshots`, `snapshots/{id}/activate`, `publish` | `expected_revision` + stabilny `Idempotency-Key` na szkic; 409 `stale_revision` zachowuje tekst i wymaga ponownego odczytu. Eksport pokazuje `files[]`, patch i komendę `guidefold proposals apply <export_id> --write`; publikacja pollowana co 2 s do stanu terminalnego. Tabela kolejki ma kolumnę „Decided by” (`ProposalSummary.decision.actor` od 1.3.0; „Undecided” bez decyzji, „Unknown” gdy `actor` jest `null`). Od 2026-09-13 (ADR-0047, kontrakt 1.11.0 §4.10) odczyty idą do `{org_base}` gdy `?repo=` jest puste — cała organizacja w zasięgu czytającego — a do `{repo_base}` po wyborze repozytorium w railu; wiersze niosą `repo_id`. Decyzja i eksport idą do `{repo_base}` z `repo_id` propozycji. |
| Usage & quality | `usage`, `usage/export`, `usage/queue/{id}/decision` | Kolejka „Needs review” pierwsza; `helped_ratio: null` to Unknown, `small_sample` pokazuje liczby zamiast procentu, brak zdarzeń to „No observations”. Zdecydowana pozycja pokazuje też `decision.actor` (od 1.3.0; „Unknown” gdy wiersz zapisał worker, nie owner). Od 2026-09-13 (ADR-0047, kontrakt 1.11.0 §4.10) odczyty idą do `{org_base}` gdy `?repo=` jest puste — cała organizacja w zasięgu czytającego — a do `{repo_base}` po wyborze repozytorium w railu; wiersze niosą `repo_id`. Decyzja na pozycji kolejki idzie do `{repo_base}` z `repo_id` pozycji; eksport ma końcową kolumnę `repo_id`. |
| Organization | `members`, `invitations`, `installations`, `auth/device/*`, `audit`, `me/identities/link/start` | Sekrety pokazywane raz, nigdy nie zapisywane. Audit to trzecia zakładka (`GET .../audit?cursor`), paginacja kursorem, pusty stan „No audit entries”; od 1.3.0 dostępna każdej roli — filtr po `actor` jest po stronie serwera, nie klienta, więc member i owner wołają ten sam endpoint, a eyebrow paneli mówi, czyje wiersze to są („Owner” / „Your own actions”). Sugestie łączenia tożsamości (`/me`'s `link_suggestions`) pokazywane w zakładce Members dla każdego użytkownika — nigdy automatycznie łączone; akcja startuje flow i przekierowuje na `login_url` zwrócone przez API. Zakładka Integrations otwiera się panelem „Set up an adapter” (pięć komend CLI, treść z [HOWTO-adapter](../docs/HOWTO-adapter.md)) nad listą Installations; członkowie widzą te same kroki, tylko owner ma link do Create an installation. |
