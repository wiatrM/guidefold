...
| Library | `skills`, `skills/facets`, `skills/facets/lookup` | Filtry `q, scope, owner, layer, status` i `cursor` w URL; bez pobierania body. Aktywna wartość ma własny lookup; `filters[*].available:false` renderuje jawny brak, nigdy cichego „All”. Od 2026-09-13 (ADR-0047, kontrakt 1.9.0 §4.10) odczyty idą do `{org_base}` gdy `?repo=` jest puste — cała organizacja w zasięgu czytającego — a do `{repo_base}` po wyborze repozytorium w railu; wiersze niosą `repo_id`. |
| Map | `map/repository`, `map/scopes`, `map/layers`, `map/relations`, `modules/{scope}` | Leniwe rozwijanie gałęzi (≤100 obiektów na żądanie, twardy limit renderu 200), tekstowa lista relacji z etykietą typu, jawne `truncated`, nazwany unmapped scope. Od 2026-09-13 (ADR-0047, kontrakt 1.9.0 §4.10) odczyty idą do `{org_base}` gdy `?repo=` jest puste — cała organizacja w zasięgu czytającego — a do `{repo_base}` po wyborze repozytorium w railu; wiersze niosą `repo_id`. Korzeń drzewa bez repozytorium to lista repozytoriów (`kind: repository`); wybór scope'u dopisuje `repo` do adresu (identyfikator scope'u jest unikalny w repozytorium; `scope_ambiguous` przy bezpośrednim linku). |
| Skill | `skills/{id}`, `revisions/{rev}`, `revisions/{rev}/raw`, `revisions/{rev}/feedback` | Brak rewizji to `revision_not_found`, nigdy podmiana na nowszą. Link źródła buduje się z `source.url`; bez `git_host_url` pokazujemy ścieżkę i „Source host not configured”. Od 2026-09-13 (ADR-0047, kontrakt 1.9.0 §4.10) odczyty idą do `{org_base}` gdy `?repo=` jest puste — cała organizacja w zasięgu czytającego — a do `{repo_base}` po wyborze repozytorium w railu; wiersze niosą `repo_id`. Feedback idzie do `{repo_base}` z `repo_id` skilla. |
| Proposals | `proposals`, `proposals/{id}`, `decision`, `export`, `publication`, `snapshots`, `snapshots/{id}/activate`, `publish` | `expected_revision` + stabilny `Idempotency-Key` na szkic; 409 `stale_revision` zachowuje tekst i wymaga ponownego odczytu. Eksport pokazuje `files[]`, patch i komendę `guidefold proposals apply <export_id> --write`; publikacja pollowana co 2 s do stanu terminalnego. Tabela kolejki ma kolumnę „Decided by” (`ProposalSummary.decision.actor` od 1.3.0; „Undecided” bez decyzji, „Unknown” gdy `actor` jest `null`). Od 2026-09-13 (ADR-0047, kontrakt 1.9.0 §4.10) odczyty idą do `{org_base}` gdy `?repo=` jest puste — cała organizacja w zasięgu czytającego — a do `{repo_base}` po wyborze repozytorium w railu; wiersze niosą `repo_id`. Decyzja i eksport idą do `{repo_base}` z `repo_id` propozycji. |
| Usage & quality | `usage`, `usage/export`, `usage/queue/{id}/decision` | Kolejka „Needs review” pierwsza; `helped_ratio: null` to Unknown, `small_sample` pokazuje liczby zamiast procentu, brak zdarzeń to „No observations”. Zdecydowana pozycja pokazuje też `decision.actor` (od 1.3.0; „Unknown” gdy wiersz zapisał worker, nie owner). Od 2026-09-13 (ADR-0047, kontrakt 1.9.0 §4.10) odczyty idą do `{org_base}` gdy `?repo=` jest puste — cała organizacja w zasięgu czytającego — a do `{repo_base}` po wyborze repozytorium w railu; wiersze niosą `repo_id`. Decyzja na pozycji kolejki idzie do `{repo_base}` z `repo_id` pozycji; eksport ma końcową kolumnę `repo_id`. |
