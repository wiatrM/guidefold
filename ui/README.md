...
| Usage & quality | `usage`, `usage/export`, `usage/queue/{id}/decision` | Kolejka „Needs review” pierwsza; `helped_ratio: null` to Unknown, `small_sample` pokazuje liczby zamiast procentu, brak zdarzeń to „No observations”. Zdecydowana pozycja pokazuje też `decision.actor` (od 1.3.0; „Unknown” gdy wiersz zapisał worker, nie owner). |
