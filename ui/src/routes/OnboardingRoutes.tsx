  ...
  return <>
    ...
    <Panel title="Import result" eyebrow={'Import ' + shortId(status.import_id)} icon={<FileCodeIcon weight="regular" aria-hidden="true" />}
      ...
      <dl className={styles.statStrip}>
        ...
        <div className={styles.stat}><dt>Publication</dt><dd><StateBadge tone={publication?.state === 'failed' ? 'error' : publication?.state === 'published' ? 'system' : 'neutral'}>{publication?.state ?? 'none'}</StateBadge>{/* Contract 1.16.0: the snapshot carries the files this import could parse and not the ones it could not. */}{publication?.partial === true && <> published without the files that failed to parse</>}</dd></div>
      </dl>
