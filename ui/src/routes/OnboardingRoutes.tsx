...
function PartialManifest({ skills, ctx }: RouteProps & { skills: Skill[] }) {
...
export function ImportRoute({ ctx }: RouteProps) {
  const {fixture, sourceURL, visibleSkills} = ctx.data;
  ...
  async function signIn(event: FormEvent<HTMLFormElement>) {
    ...
    await ctx.source.startLogin(provider, '');
    ctx.go('import', { step: 'organization' });
  }
  async function createOrganization(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!ctx.canWrite) return;
    await ctx.source.createOrg({ name: fixture.org, slug: fixture.org }, 'fixture-org');
    ctx.go('import', { step: 'preview' });
  }
  async function simulateImport() {
    if (!ctx.canWrite) return;
    await ctx.source.createImport({ org: fixture.org, repo: fixture.repo }, null, 'fixture-import');
  ...
  return <div className={styles.route}>
    <ol className={styles.steps} aria-label="Fixture import progress">
      {steps.map((item, index) => {
        ...
        const body = <>
          ...
          {completed && <span className={styles.stepDone}><CheckCircle weight="regular" aria-hidden="true" />Done</span>}
        </>;
        return <li key={item.id} className={completed ? styles.completedStep : current ? styles.currentStep : undefined} aria-current={current ? 'step' : undefined}>
          {index <= currentStep ? <Link to={ctx.href('import', { step: item.id })} className={styles.stepLink}>{body}</Link> : body}
        </li>;
      })}
    </ol>
    <AccessNote ctx={ctx} />
    ...
    {partial && <PartialManifest ctx={ctx} skills={skills} />}
  </div>;
}
...
export function OrganizationRoute({ ctx }: RouteProps) {
  ...
  async function addMember(event: FormEvent<HTMLFormElement>) {
    ...
    await ctx.source.inviteMember(fixture.org, { email: value, role: 'member' }, 'fixture-invite-' + value);
    setLabel('');
    setMemberError('');
  ...
  async function removeMember(index: number) {
    if (!ctx.canWrite) return;
    await ctx.source.removeMember(fixture.org, 'local-' + index, 'fixture-remove-' + index);
    setMemberStatus('Local member entry removed. No real access changed.');
  }
  async function checkConnection() {
    if (!ctx.canWrite) return;
    await ctx.source.listInstallations(fixture.org);
  ...
  async function changeToken() {
    if (!ctx.canWrite) return;
    if (ctx.memory.token) await ctx.source.revokeInstallation(fixture.org, 'fixture-installation', 'fixture-revoke');
    else await ctx.source.createInstallation(fixture.org, { name: 'Local scenario token', scopes: ['search', 'use', 'events'] }, 'fixture-installation');
    setIntegrationStatus('Token lifecycle simulated. No credential was issued or revoked.');
  }
  const integrationDone = Boolean(integrationStatus || ctx.memory.connection);
  const integrationMessage = integrationStatus || (ctx.memory.connection ? 'Local check complete. Actual adapter connection and delivery remain Unknown.' : 'No local connection check has been run.');
  ...
  return <div className={styles.route}>
    ...
    {tab === 'members' ? <div className={styles.asideColumns}>
      <Panel title="Members" eyebrow="Organization access" icon={<Users weight="regular" aria-hidden="true" />}>
        <StateBadge>Local scenario</StateBadge>
        ...
        <p className={styles.feedback} role="status">{memberStatus && <><CheckCircle weight="regular" aria-hidden="true" /><span>{memberStatus}</span></>}</p>
      </Panel>
      ...
      <div className={styles.twoColumns}>
        <Panel title="Install an adapter" eyebrow="Proposed CLI" icon={<Terminal weight="regular" aria-hidden="true" />}>
          <p>This is a local simulation: the prototype does not install an adapter, contact a harness or run a real connection check. Commands below are proposed product syntax.</p>
          ...
          <div className={styles.actionFooter}><ActionButton tone="human" disabled={!ctx.canWrite} onClick={checkConnection}><LinkSimple weight="regular" aria-hidden="true" />Simulate connection check</ActionButton><p className={styles.feedback} role="status">{integrationDone ? <><CheckCircle weight="regular" aria-hidden="true" /><span>{integrationMessage}</span></> : integrationMessage}</p></div>
        </Panel>
...
// ---------------------------------------------------------------------------
// Hosted API routes (F11, F12, F19). No fixture value reaches these components.
// ---------------------------------------------------------------------------
