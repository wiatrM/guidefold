...
const ADAPTER_STEPS: { title: string; detail: string; command: string }[] = [
  ...
  {
    title: 'Telemetry',
    detail: 'On by default once this sign-in has a token and an endpoint (owner decision, ADR-0047) — queued SEARCH/USE events are sent automatically in the background after a harness call, no manual flush and no hook-path network call. Switch it off any time with guidefold telemetry disable (or GUIDEFOLD_TELEMETRY=0 for one job); GUIDEFOLD_TELEMETRY_DISABLE stops local collection entirely.',
