...
import {bodyPrefix, digest} from '../domain/skill';
...
import type {Params, Proposal, ProposalStage, RouteContext, View} from '../domain';
...
export function ProposalsRoute({ctx}: Props) {
  const {fixture, proposalSkill, sourceURL, defaultProposal} = ctx.data;
  ...
  return <div className={styles.route}>
    ...
    <section className={styles.summary} aria-label="Proposal status and evidence">
      <div className={styles.summaryHeading}>
        ...
        <span className={styles.eyebrow}>{stageNames[stage]}</span>
      </div>
      <p>Review the existing skill file as a candidate revision. This fixture always shows the same one candidate, not a live review queue, and introduces no new procedure or source problem.</p>
      ...
      <Panel title="Decision and publication" eyebrow="Local fixture workflow" icon={<GitBranch aria-hidden="true" />}>
        <div className={styles.decisionContent}>
          ...
          <ol className={styles.lifecycle} aria-label="Publication lifecycle">
            {(['draft', 'approved_for_export', 'awaiting_git', 'published'] as const).map(item => <li key={item} aria-current={stage === item ? 'step' : undefined}>{stage === item ? <StateBadge tone="human">{stageNames[item]}</StateBadge> : stageNames[item]}</li>)}
          </ol>
...
export function UsageRoute({ctx}: Props) {
  const {fixture, proposalSkill, findSkill} = ctx.data;
  ...
  // This fixture ships no adapter events by design (04-wireframes §6): every measure below is
  // Unknown for every filter, so the page states that once instead of on three separate tiles.
  const activeFilters: {label: string; value: string}[] = [];
  if (params.get('scope')) activeFilters.push({label: 'Scope', value: params.get('scope')!});
  if (params.get('skill')) activeFilters.push({label: 'Skill', value: findSkill(params.get('skill'))?.name ?? params.get('skill')!});
  if (params.get('revision')) activeFilters.push({label: 'Revision', value: params.get('revision')!});
  if (params.get('harness')) activeFilters.push({label: 'Harness', value: params.get('harness')!});
  const clearFiltersHref = ctx.href('usage', {scope: null, skill: null, revision: null, harness: null});
  return <div className={styles.route}>
    ...
    <Panel title="Usage evidence" icon={<ListChecks aria-hidden="true" />}>
      <p>No adapter events are recorded for this fixture repository. Delivery, helpfulness and observation coverage stay Unknown until an adapter reports them — a missing report is not a zero result.</p>
    ...
    <Panel title="Filters" eyebrow="Scope, skill, revision and harness" icon={<FileText aria-hidden="true" />}>
      ...
      {activeFilters.length > 0 && <div className={styles.contextSummary}>
        ...
        <ActionButton href={clearFiltersHref}>Clear filters</ActionButton>
      </div>}
      <ProvenanceTrail entries={[
        {label: 'Window', value: 'Unknown'},
        {label: 'Source', value: 'no event ledger'},
        {label: 'Coverage', value: 'Unknown'},
      ]} />
    ...
    <Panel title="Delivery and outcome evidence" icon={<ListChecks aria-hidden="true" />}>
      <DataTable caption="Delivery and outcome evidence" headings={['Measure', 'Value', 'Why it is unknown']}>
        {evidenceRows.map(([measure, why]) => <tr key={measure}><th scope="row">{measure}</th><td className={styles.muted}>Unknown</td><td>{why}</td></tr>)}
      </DataTable>
    ...
    <Panel title="Review queue" icon={<ListChecks aria-hidden="true" />}>
    ...
    <FixtureSkillHealth ctx={ctx} />
  </div>;
}

/** The fixture has no adapter events, so the health panel reads a simulated ledger and says so. */
function FixtureSkillHealth({ctx}: Props) {
  const simulated = useAsync(() => ctx.source.getUsage({org: ctx.data.fixture.org, repo: ctx.data.fixture.repo}, {}), 'fixture-usage');
  if (simulated.phase !== 'ready' || !simulated.value || !simulated.value.skills.length) return null;
  return <SkillHealthPanel skills={simulated.value.skills} queue={simulated.value.queue} sort={ctx.params.get('sort') ?? ''} href={ctx.href} go={ctx.go}
    skillHref={item => ctx.href('skill', {skill: item.skill_id, revision: null, tab: 'content', from: 'usage'})}
    eyebrow="Meridian fixture · simulated ledger for this panel only, not adapter evidence" />;
}
...
// ---------------------------------------------------------------------------
// Hosted API routes (F16, F17, F18). No fixture value reaches these components.
// ---------------------------------------------------------------------------
