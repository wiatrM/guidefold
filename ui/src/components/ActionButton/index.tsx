...
import {MorphButton as Button} from '../spectrumui/morph-button';
...
export function ActionButton(props:ButtonProps){
 ...
 if(props.href){
  ...
  const shared={...rest,className,'data-slot':'button','aria-disabled':disabled||undefined,tabIndex:disabled?-1:rest.tabIndex,onClick:(e:React.MouseEvent<HTMLAnchorElement>)=>{if(disabled)e.preventDefault();else rest.onClick?.(e);}};
 ...
 return <Button {...rest} type={rest.type||'button'} className={className} data-slot="button">{children}</Button>;
}
