...
| `live` | The Live Agent's API half (ADR-0046): `gfm.live_runs`, `live_run_targets`, `live_run_events`, and the five `{org_base}/live/*` routes — start (owner, one active run per organisation), list, one run with its targets, the append-only event log polled with a position cursor, and cancel. It enqueues `live.plan` only; `live.repo` and the model call belong to the worker, which calls this package's `Append`, `SetTargetState` and `Finish` to make progress visible. It never opens the organisation's model key — only checks that `gfm.org_credentials` holds one. |
| `ghapp` | The only adapter that talks to `api.github.com`: signs the GitHub App's own RS256 JWT from `GITHUB_APP_PRIVATE_KEY_FILE` (stdlib `crypto/rsa` only, no new module dependency), exchanges it for an installation token cached in memory with a five-minute refresh margin, and offers a read half and a narrow write half. Read: `ListSkillFiles` (the git trees API, `guidefold.yaml` and `AGENTS.md` at the root, and `**/.agents/skills/**/SKILL.md`) and `ReadFile` (the contents API, 256 KiB ceiling) — `ascend.run` (ADR-0036) and `live.repo` (ADR-0046) both read through it. Write, added for the coverage-bot amendment to ADR-0036 (points 1, 1a): `UpsertStickyComment` (one PR comment, rewritten per push, found by a marker across paginated listings), `CreateBranchCommit` (a commit against Guidefold's own branch, built through the git data API — resolve, blob, tree with `base_tree`, commit, ref — never a clone or the `git` binary, and refusing any path outside `AGENTS.md`/`.agents/skills` with `ErrPathNotAllowed`) and `OpenPullRequest` (idempotent: returns an existing PR's URL instead of erroring). None of the three ever touches the customer's own branch or source. |
...
| `model` | A small streaming chat client over the three providers an organisation may hold a key for (ADR-0045): `openrouter`, `anthropic`, `openai` — one file per provider concern (`chatcompletions.go` shared by the first two, `anthropic.go` its own shape), `Stream(ctx, req, onDelta)` over a callback so a caller batches deltas its own way, usage read from the provider's response and marked `Estimated` when absent, and four named errors (`ErrQuotaExhausted`, `ErrRateLimited`, `ErrModelNotAvailable`, `ErrUnauthorized`) a caller branches on. The organisation's key travels on `Request.APIKey` for one call and never appears in a returned value or an error string. |
| `agentrun` | The worker half of the Live Agent (ADR-0046) and of the GitHub App's pull-request coverage report (ADR-0036 points 1a, 4a): `live.plan` (resolve targets, one `live.repo` job each, `github_app_not_configured` for a repository with no installation — reported, never dropped), `live.repo` (read `guidefold.yaml`, `AGENTS.md` and every `SKILL.md` through `ghapp` — a repository with no `guidefold.yaml` is imported under a scope map inferred from its skill directories and CODEOWNERS (ADR-0050); only a declared map too large to read skips the target, with `guidefold_yaml_unreadable` — build an import through `importer`'s own in-process seam, then drive `review`'s own `GenerateProposals` seam for `kind=consolidation`, waiting on each child job and appending an event at every transition; it never calls a model itself, that happens inside `proposal.generate` under the organisation's own key), and `pr.report` (name the published rules that apply to a pull request's changed paths, ask the model whether the diff contradicts one, upsert the sticky comment through `ghapp`, enqueue `ascend.run` when a skill file changed — a missing key or an exhausted quota still produces a comment, and a quota failure is never retried). Resolves a repository to its GitHub installation from `gfm.repos.git_host_url` and `gfm.github_installations.repositories`, since no column links the two directly yet. |
