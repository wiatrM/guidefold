...
// precedence as `guidefold.yaml` > approved proposal > inferred, so a row this
// function wrote is overwritten by the next import that carries the file — and
// a row the file wrote is *not* protected here, because the owner approving a
// map for it is a later, explicit decision than the file being read.
...
func (ScopeMapWriter) ApplyScopeMap(ctx context.Context, tx pgx.Tx, orgID, proposalID,
 ...
 ON CONFLICT (org_id,repo_id,scope) DO UPDATE SET
  ...
  reviewed_by=EXCLUDED.reviewed_by,proposal_id=EXCLUDED.proposal_id,updated_at=now()`,
			...
			SourceLLMApproved, optional(reviewerID), optional(proposalID)); e != nil {
			return written, e
		}
		written++
	}
