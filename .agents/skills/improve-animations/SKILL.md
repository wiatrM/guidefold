...
The rule catalog with precise values lives in [AUDIT.md](./resources/AUDIT.md). The plan format lives in [PLAN-TEMPLATE.md](./resources/PLAN-TEMPLATE.md). Load them when you audit and when you write plans.
...
Audit against the eight categories in [AUDIT.md](./resources/AUDIT.md):
...
One plan per selected finding, using [PLAN-TEMPLATE.md](./resources/PLAN-TEMPLATE.md), written into `plans/` as `NNN-short-slug.md` (monotonic numbering; respect existing plans). Stamp each plan with the current commit (`git rev-parse --short HEAD`).
