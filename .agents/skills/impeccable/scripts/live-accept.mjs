 ...
 * successful ack, and resources/live.md then tells the agent to finish the edit
 ...
 * resources/live.md routed the agent to "read file, find markers, edit".
 *
...
Output (JSON):
  ...
  if (priorReceipt) {
    ...
    console.log(JSON.stringify(sameOperation
      ...
      : {
          ...
          // agent_done and resources/live.md tells the agent to "read file, find
  ...
  if (svelteComponentManifest) {
    ...
    if (result.carbonize) {
      result.todo = 'REQUIRED before next poll: carbonize cleanup in ' + result.file + '. See resources/live.md "Required after accept".';
    }
  ...
  if (isDiscard) {
    ...
    // five-step checklist lives in resources/live.md (loaded once per
    // session); repeating it per-event would waste tokens.
    if (result.carbonize) {
      result.todo = 'REQUIRED before next poll: carbonize cleanup in ' + relFile + '. See resources/live.md "Required after accept".';
    }
