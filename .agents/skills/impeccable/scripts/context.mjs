...
const SKILL_REFERENCE_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'resources');
...
async function cli() {
  ...
  if (!ctx.hasProduct) {
    ...
    const parts = ctx.hasVisualImplementation
      ? [
          ...
          'For `init`, `teach`, `shape`, or any request to create a new surface or replacement visual world, load resources/init.md and create PRODUCT.md with the user first. ' +
          'After init writes PRODUCT.md, resources/new-work.md preserves and documents the incumbent system for an ' +
          ...
          'simulated user. Init writes product truth only; resources/new-work.md owns every visual decision.',
      ...
      : [
          ...
          'resources/init.md, complete its human or structured simulated-user interview, and write PRODUCT.md before ' +
          ...
          'must finish resources/init.md for PRODUCT.md, then resources/new-work.md establishes the world and surface. Scoped ' +
  ...
  if (!ctx.hasDesign) {
    parts.push(ctx.hasVisualImplementation
      ? 'INCUMBENT_WORLD_UNDOCUMENTED: PRODUCT.md exists and DESIGN.md is missing, but code contains incumbent visual decisions. ' +
        'For shape or a new-surface/redesign request, load resources/new-work.md: an extension documents and preserves the code-defined world; ' +
      ...
      : 'WORLD_DISCOVERY_REQUIRED: PRODUCT.md exists but no DESIGN.md or incumbent visual implementation was found. ' +
        'For a new build or redesign, load resources/new-work.md and establish the visual world with the human or structured ' +
  ...
  for (const reference of platformReferences) {
    parts.push(
      `# NATIVE PLATFORM REFERENCE: ${reference.name.toUpperCase()} (resources/${reference.name}.md)\n\n${reference.content.trim()}`,
    );
  }
  appendImageToolsDirective(parts);
  appendStalenessDirective(parts, ctx, cliOptions);
...
// resources/craft-floor.md carries the detector-blind reflexes on every build,
