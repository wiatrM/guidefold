...
- [The contract (read once)](#the-contract-read-once)
- [Poll loop](#poll-loop)
- [Start](#start)
...
- [Handle `manual_edit_apply`](#handle-manual_edit_apply)
- [Exit](#exit)
- [Cleanup](#cleanup)
...
`event.action` is `impeccable` (freeform): work from resources/core.md's design rules plus [craft-floor.md](craft-floor.md); decide the visitor mode from the surface; do not load a sub-command reference. Freeform is not a pass to skip parameters: follow the budget and freeform bias in section 7. Any other action (`bolder`, `quieter`, `distill`, `polish`, `typeset`, `colorize`, `layout`, `adapt`, `animate`, `delight`, `overdrive`): read `resources/<action>.md` before planning; its MUST params layer on top of the section 7 budget.
