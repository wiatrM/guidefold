...
|------|------------|------|
| `spool/<tenant\|local>/<environment>/events-<UTC date>.jsonl` | `find`, `hook`, `load` | E6.4/E2.7 SEARCH/USE contract events (`docs/SEARCH-USE-TELEMETRY.md`): `search_requested`, `search_results`, `card_injected`, `skill_load_requested`, `skill_load_completed`, `telemetry_health` — and nothing else: the spool is the ledger's queue, so an event type outside the frozen `event_types` of `services/search/telemetry-schema.json` is never written to it, because ingest can only answer `unknown_event_type` (D19, 2026-09-15). `search_results` also carries `backend` (`online_sparse`/`local_sparse`/`none`) and `fallback_reason` (the E2.6 degradation reason, e.g. `timeout`/`connection`/`http_5xx`/`auth`/`invalid_response`/`config`, or `null`). Append-only, bounded (10 MB / 7 days, oldest-first drop, counted in `telemetry_health.dropped`). No raw prompt text, no bearer token; an optional tenant-scoped keyed HMAC of the query only, rotated monthly. |
...
| `parity.json` | `find`, `hook`, `load` on `backend: service` | The E2.9 runtime parity counter, a LOCAL diagnostic that is never uploaded: `{compared, mismatched, last}`, where `last` holds the two 16-hex-char SHA-256 selected-set hashes and the `search_id` of the most recent disagreement — never the query text, a card body, a description or a URN. `guidefold doctor` reports it as `search-parity`. A mismatch is expected (two rankers, two scales), so it is a `warn` that names the count, never a `fail`. |
