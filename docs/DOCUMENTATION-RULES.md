...
| Stan implementacji pivotu | [PIVOT-IMPLEMENTATION](PIVOT-IMPLEMENTATION.md) | Które P01–P16 mają kod i testy, mapa modułów Go, luki przed pilotem; nie dowód pilota ani zamiennik backlogu. |
| Kryteria kill, partner projektowy | [PRODUCT-FOCUS](PRODUCT-FOCUS.md) | Kryteria kill do 2026-09-20 i sekcja „Design partner"; nie wiąże już kolejności `docs/BACKLOG.md` (historyczny) — kolejność jest w PIVOT-BACKLOG. |
| Wdrożenie produkcyjne | [deploy/k8s/environments/cloudfloo-io/README.md](../deploy/k8s/environments/cloudfloo-io/README.md), [ADR-0030](adr/ADR-0030-immutable-service-releases-on-kubernetes.md) | Rejestr wydań i runbook ArgoCD/Kubernetes dla `cloudfloo-io`; obrazy niemutowalne per release. Reguła „produkcja jest święta" z CLAUDE.md obowiązuje ponad tym plikiem. |
| Multi-repo organizacja, konfigurator CI | [ADR-0042](adr/ADR-0042-multi-repo-organisation-and-ci-configurator.md), [docs/superpowers/specs](superpowers/specs) | Specyfikacja towarzysząca ADR-0042 (P16): `gfm.repo_links`, ustawienia generatora per org, konfigurator CI; na `main` bez kodu. |
| Pre-rejestracja badania przed zebraniem danych (hipotezy, jednostka analizy, moc, stop rules) | [PREREG harness-conditional evolution](reports/research/PREREG-harness-conditional-evolution-2026-09-15.md) | Wzorzec i bieżąca pre-rejestracja badania „ewolucja biblioteki per harness”, zamrożona sha256 przed pierwszą sesją; raporty badawcze idą do `docs/reports/research/`. Nie zastępuje [E6.7-PROTOCOL](pilot/E6.7-PROTOCOL.md) ani rubryki P15 i nie zmienia rankingu (ADR-0029 reguła 2). |
| Stan repo wobec MVP, co zostało do domknięcia | [raport 2026-09-15](reports/product/2026-09-15-mvp-closure-status.md) | Bieżąca lista krytycznej ścieżki z dowodem przy każdym twierdzeniu; poprzedza go [audyt 2026-09-12](reports/product/2026-09-12-assumptions-vs-implementation-audit.md) (lista PRIO, materiał datowany). |
## Pipeline UI
...
Wszystkie 81 skilli w `.agents/skills/` (`python3 tools/check_skills.py`, stan 2026-09-15) jest zindeksowanych w AGENTS.md: skille reguł (kierunek produktu, zasady inżynierskie, jakość, konwencje, UI) wynikają z [ADR-0032](adr/ADR-0032-engineering-principles-and-hexagonal-architecture.md), pozostałe to przyjęte skille design/motion/generacji treści indeksowane osobną tabelą; Claude Code czyta je przez linki w `.claude/skills/`, a `.claude/settings.json` uruchamia hooki opisane w [.claude/README.md](../.claude/README.md). Skille projektu rozwijają reguły odczytu i wykonania, nie są drugim PRD. Dystrybuowany [bootstrap](../skills/guidefold/SKILL.md) służy repozytoriom klientów; nie dopisuj do niego wewnętrznych planów Guidefold ani nie zmieniaj skilli fixture w celu aktualizacji instrukcji projektu.
