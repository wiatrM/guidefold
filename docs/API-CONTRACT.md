...
Status: obowiązujący kontrakt implementacyjny, `contract_version: 1.5.0`. Data: 2026-09-12.
...
### 4.9 Live Agent (ADR-0046 — przebieg na żądanie po podpiętych repozytoriach)

| Metoda | Ścieżka | Rola / zakres | Wejście | Wyjście | Błędy | Idem. | P / AC |
|---|---|---|---|---|---|---|---|
| POST | `{org_base}/live/runs` | owner + CSRF | `{prompt:str!, provider:str?, model:str?, repos:[]str?}` | `202` `LiveRun` | `invalid_prompt`, `invalid_provider`, `invalid_model`, `model_credential_missing`, `live_run_already_active`, `secret_encryption_unavailable` | tak | ADR-0046 / U4 |
...
| `LiveRunEvent` | `seq:int!`, `at:ts!`, `repo_id:str?`, `type:{run.started\|repo.started\|model.delta\|finding\|repo.finished\|run.finished\|error}!`, `payload:obj!` | `seq` rośnie monotonicznie w obrębie przebiegu i jest kursorem `after`. `model.delta` niesie `{text:str!}` zbatchowany przez workera (~500 ms albo ~2 KB), żeby gadatliwy model nie zrobił z jednego przebiegu stu tysięcy wierszy. `finding` niesie `{scope:str?, path:str?, summary:str!, severity:{info\|warn}!}` i jest jedynym typem, który UI pokazuje poza transkryptem. Dziennik jednego przebiegu ma sufit **20 000 zdarzeń**: po jego osiągnięciu worker przestaje dopisywać `model.delta`, dopisuje jedno zdarzenie `error` z powodem `live_run_log_truncated` i dalej zapisuje wyłącznie zdarzenia strukturalne (`repo.*`, `finding`, `run.finished`). Przebieg nie jest przerywany, a UI mówi wprost, że transkrypt jest ucięty — cicha utrata linii transkryptu wyglądałaby jak agent, który zamilkł. |
...
Repozytorium jest wiązane z instalacją aplikacji GitHub przez `gfm.repos.git_host_url` dopasowany do `full_name` w `gfm.github_installations.repositories`. To jest **obejście**, nie docelowy kształt: ADR-0034 przewiduje jawne połączenie repozytorium GitHuba z `repo_id` jako akt właściciela w UI, a `GitHubInstallation.repositories[].repo_id` istnieje w §5.1 właśnie po to. Dopóki to połączenie nie jest zapisywane, repozytorium bez rozpoznawalnego adresu hosta dostaje cel `skipped` z `github_app_not_configured` — nazwany, nie pominięty.

`live.repo` nie narzuca modelowi schematu JSON, jak robi to `proposal.generate`. Zdarzenia `finding` powstają z linii odpowiedzi w formie `FINDING: {info|warn} <jedno zdanie>`; wszystko inne jest transkryptem. Luźniejsza umowa jest tu celowa: przebieg ma być czytelny dla człowieka na żywo, a nie wyprodukować rekord do zatwierdzenia, więc model, który nie trafi w konwencję, traci ustrukturyzowane ustalenie, a nie cały przebieg.

`live.plan` i `live.repo` (ADR-0046) **czytają** repozytorium przez API treści GitHuba tokenem instalacji — wyłącznie `AGENTS.md` i `**/.agents/skills/**/SKILL.md`. Nie klonują, nie uruchamiają niczego z repozytorium, nie zakładają gałęzi ani PR-a; to, co agent proponuje zmienić, idzie do istniejącego przepływu propozycji. Klucz modelu należy do organizacji (ADR-0045), jest otwierany w pamięci na czas jednego joba i nigdy nie trafia do `payload`, `checkpoint`, `result`, logu ani komunikatu błędu. Wyczerpanie `max_usd` albo `max_tokens` kończy przebieg jako `partial` z `live_run_budget_exhausted`, po zapisaniu wszystkich dotychczasowych zdarzeń. Anulowanie zatrzymuje `live.repo` na najbliższym checkpoincie, a nie na końcu repozytorium. Polityka sieciowa workera musi mieć wyjście do endpointu OpenRoutera obok GitHuba; do czasu tej zmiany job kończy się `skipped` z `github_app_not_configured`.
