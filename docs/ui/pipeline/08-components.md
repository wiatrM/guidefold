...
Siedemnasty komponent `RepositoryFilter` (2026-09-13, [ADR-0047](../../adr/ADR-0047-organisation-is-the-default-read-scope.md)) to jedyny selektor repozytorium konsoli: natywny `<select>` z etykietą (Base UI `Select` zawiesza jsdom i zmieniałby adres przy nawigacji strzałkami), opcja „All repositories” plus lista z `GET {org_base}/repos`, wartość z `?repo=`; zamontowany raz w railu powłoki (`app.tsx`), nigdy w trasie. Wartość spoza listy pozostaje widoczna zamiast cichego resetu do „All”.
Osiemnasty komponent `OrgSwitcher` (2026-09-13, polecenie właściciela, [UX §3a](../UX.md#3a-wdrożenie-organizacji-i-repozytoria-z-githuba)) zastępuje dawną statyczną etykietę „Workspace” i slug w nagłówku raila: `DropdownMenu` (Base UI) na `SidebarMenuButton`, jak `UserDropdown` obok niego. Otwarcie listuje każde `me.orgs` z rolą, zaznacza bieżącą organizację i kończy się „Create organization” do `/import?step=organization`; wybór zostawia bieżący widok, podmienia `org` zwykłą nawigacją (Wstecz wraca) i usuwa `repo`. Zapamiętany kontekst (bez `org` w URL konsola wraca do ostatnio wybranej organizacji użytkownika, jeśli nadal do niej należy; URL zawsze wygrywa) jest osobną, czystą funkcją `resolveOrg` w `ui/src/domain/orgSwitch.ts`, tam testowaną; `localStorage` jest wygodą tej przeglądarki, nie uprawnieniem. Base UI Menu zawiesza jsdom po otwarciu jak w `UserDropdown`, więc ten komponent renderuje się w Vitest tylko zamknięty; otwarcie i wybór pokrywa `ui/e2e/org-switcher.spec.ts`.
